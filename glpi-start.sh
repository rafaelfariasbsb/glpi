...
if [[ "$VERSION_GLPI" -le "$CURRENT_VERSION" ]]; then
    echo "Directory /usr/share/glpi/$VERSION_GLPI already exists or you cannot downgrade version. Skipping installation."
else
# Check if the /usr/share/glpi/public directory exists
    if [ ! -d "/usr/share/glpi/version/$VERSION_GLPI" ]; then
        echo "Directory /usr/share/glpi/version/$VERSION_GLPI does not exist. Creating..."
        ...
        if [ -e "/tmp/$TAR_GLPI" ]; then
           ...
           # create the downstream.php file
            ...
            # Ch   ange permission of the glpi and files folder
        ...
        echo "Directory /usr/share/glpi/version/$CURRENT_VERSION already exists. Skipping installation."
    fi
fi
